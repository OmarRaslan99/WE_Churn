...
### 2026-07-17 - Seance 5 : ajout d'un service frontend de demonstration

A la demande de l'enseignant, un quatrieme service `frontend` a ete ajoute pour la demonstration live. Il s'agit d'une passerelle (Backend-For-Frontend) FastAPI qui sert une page web unique et fait proxy vers `inference-svc` (`/api/predict`) et `monitoring-svc` (`/api/metrics`), plus un endpoint `/api/sample` qui tire un client au hasard dans `data/churn.csv`. Ce choix de passerelle a ete prefere a une page purement statique appelant directement les API pour trois raisons : une seule origine (pas de CORS a gerer), le service de monitoring **reste interne en ClusterIP** (inutile de l'exposer), et une seule URL NodePort a communiquer pour la demo. Le front n'introduit aucune logique metier : la prediction et la validation restent dans l'inference et le preprocessing.

L'ajout de ce service consomme du quota : le front est dimensionne a `100m/128Mi` en requests et `250m/192Mi` en limits. Le total du namespace passe donc a `1050m/960Mi` en requests et `2000m/1344Mi` en limits, toujours sous le quota `2500m/1536Mi`. La marge la plus contrainte devient la memoire en limits (`192Mi` restants), ce qui reste acceptable pour une demo mais serait le premier poste a surveiller en cas de nouvel ajout. La strategie de deploiement du front est la strategie par defaut (RollingUpdate implicite) : contrairement a l'inference, le front est sans etat et leger, donc un surge temporaire tient dans la marge. Le service est integre a la CI (build/push de l'image `we-churn-frontend`), a `docker-compose` (port 8080) et aux manifests `k8s/frontend.yaml`, et il est couvert par `tests/test_frontend.py`.

### 2026-07-17 - Seance 5 : passage a un tag d'image immuable
